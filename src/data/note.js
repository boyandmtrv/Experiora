...
const endpoints = {
    'posters': `/classes/Poster?where=${encodeURIComponent('{"readyForTest": true}')}`,
    'postersWithUser': (userId) => `/classes/Poster?where=${encodeURIComponent(`{"$or":[{"readyForTest": true}, {"owner":${JSON.stringify(createPointer('_User', userId))}}]}`)}`,
